...
b. **Extract and rename probe binaries** — shell script that:
   ...
   IMPORTANT: The CI workflows upload `install/${{ matrix.preset }}/` as the artifact path. When downloaded by `actions/download-artifact@v4`, the preset directory is stripped — each artifact directory contains `lib/qtmcp/{tag}/...` directly (NOT `ci-linux/lib/...`). Use a find command to locate the .so or .dll reliably rather than hardcoding paths.
