...
<task type="auto">
  ...
  <action>
Create three compat headers under `src/compat/`. All headers use `#pragma once`, wrap content in `namespace qtmcp::compat {}`, and contain ONLY inline functions with `#if QT_VERSION` guards inside.
