...
WalkResult AccessibilityTreeWalker::walk(QWidget* rootWidget, const WalkOptions& opts)
{
    ...
    result.tree = walkNode(rootIface, 0, opts, refCounter, result.refMap,
                           charCount, result.truncated);
    result.totalNodes = refCounter;  // Total refs assigned

    return result;
}

QJsonObject AccessibilityTreeWalker::walkNode(QAccessibleInterface* iface,
                                               int depth,
                                               ...
                                               bool& truncated)
{
    ...
    if (state.collapsed)   states[QStringLiteral("expanded")] = false;
    if (state.selected)    states[QStringLiteral("selected")] = true;
    if (state.readOnly)    states[QStringLiteral("readonly")] = true;
    if (state.pressed)     states[QStringLiteral("pressed")] = true;
    if (state.hasPopup)    states[QStringLiteral("hasPopup")] = true;
    if (state.modal)       states[QStringLiteral("modal")] = true;
    if (state.editable)    states[QStringLiteral("editable")] = true;
    if (state.multiLine)   states[QStringLiteral("multiline")] = true;
    if (state.passwordEdit) states[QStringLiteral("password")] = true;
    ...
    for (int i = 0; i < childCount; ++i) {
        ...
        QJsonObject childNode = walkNode(child, depth + 1, opts, refCounter,
                                          refMap, charCount, truncated);
