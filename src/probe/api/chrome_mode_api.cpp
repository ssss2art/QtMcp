...
QWidget* getActiveWindowWidget() {
    ...
    throw JsonRpcException(
        ErrorCode::kNoActiveWindow,
        QStringLiteral("No active Qt window found"),
        QJsonObject{
            {QStringLiteral("hint"),
             QStringLiteral("Ensure the application has a visible window")}
        });
}
...
void findMatchingNodes(QAccessibleInterface* iface, const QString& query,
                       int& refCounter, QJsonArray& matches,
...
void ChromeModeApi::registerReadPageMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.readPage"),
...
void ChromeModeApi::registerClickMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.click"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            throw JsonRpcException(
                ...
                QStringLiteral("Element %1 has no clickable interface").arg(ref),
                QJsonObject{
                    {QStringLiteral("ref"), ref},
                    {QStringLiteral("role"), roleName}
                });
        });
}
...
void ChromeModeApi::registerFormInputMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.formInput"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QString ref = p[QStringLiteral("ref")].toString();
            ...
            throw JsonRpcException(
                ErrorCode::kFormInputUnsupported,
                QStringLiteral("No supported input strategy for element %1 (role: %2)")
                    .arg(ref, roleName),
                QJsonObject{
                    {QStringLiteral("ref"), ref},
                    {QStringLiteral("role"), roleName}
...
void ChromeModeApi::registerGetPageTextMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.getPageText"),
        [](const QString& /*params*/) -> QString {
...
void ChromeModeApi::registerFindMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.find"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
...
void ChromeModeApi::registerNavigateMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.navigate"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (action == QStringLiteral("activateTab") ||
                action == QStringLiteral("activateMenuItem")) {
                QString ref = p[QStringLiteral("ref")].toString();
                if (ref.isEmpty()) {
                    throw JsonRpcException(
                        JsonRpcError::kInvalidParams,
                        ...
                        QJsonObject{
                            {QStringLiteral("method"), QStringLiteral("chr.navigate")},
            ...
            if (action == QStringLiteral("back") || action == QStringLiteral("forward")) {
                ...
                QString targetShortcut = (action == QStringLiteral("back"))
                    ? QStringLiteral("Ctrl+Z") : QStringLiteral("Ctrl+Y");
                QString altShortcut = (action == QStringLiteral("back"))
            ...
            throw JsonRpcException(
                ErrorCode::kNavigateInvalid,
                QStringLiteral("Unknown navigate action: %1").arg(action),
                QJsonObject{
                    {QStringLiteral("action"), action},
                    {QStringLiteral("validActions"),
                     QJsonArray{QStringLiteral("activateTab"),
...
void ChromeModeApi::registerTabsContextMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.tabsContext"),
        [](const QString& /*params*/) -> QString {
            ...
            for (QWidget* w : topLevels) {
                ...
                info[QStringLiteral("geometry")] = QJsonObject{
                    {QStringLiteral("x"), geom.x()},
...
void ChromeModeApi::registerReadConsoleMessagesMethod() {
    m_handler->RegisterMethod(QStringLiteral("chr.readConsoleMessages"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
