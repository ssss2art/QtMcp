    ...
    if (x < 0 || y < 0 || x >= winSize.width() || y >= winSize.height()) {
        throw JsonRpcException(
            ...
            QStringLiteral("Coordinates (%1, %2) out of bounds for window size (%3 x %4)")
                .arg(x).arg(y).arg(winSize.width()).arg(winSize.height()),
            QJsonObject{
                {QStringLiteral("x"), x},
                {QStringLiteral("y"), y},
...
void ComputerUseModeApi::registerScreenshotMethods() {
    m_handler->RegisterMethod(QStringLiteral("cu.screenshot"),
...
void ComputerUseModeApi::registerMouseMethods() {
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.click"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.rightClick"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.middleClick"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.doubleClick"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.mouseMove"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.drag"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
            ...
            auto checkBounds = [&](const QPoint& pt, const QString& label) {
                if (pt.x() < 0 || pt.y() < 0 ||
                    pt.x() >= winSize.width() || pt.y() >= winSize.height()) {
                    throw JsonRpcException(
                        ErrorCode::kCoordinateOutOfBounds,
                        QStringLiteral("%1 coordinates (%2, %3) out of bounds for window size (%4 x %5)")
                            .arg(label).arg(pt.x()).arg(pt.y())
                            .arg(winSize.width()).arg(winSize.height()),
                        QJsonObject{
                            {QStringLiteral("x"), pt.x()},
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.mouseDown"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.mouseUp"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
...
void ComputerUseModeApi::registerKeyboardMethods() {
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.type"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (!focusWidget) {
                throw JsonRpcException(
                    ...
                    QStringLiteral("No widget has keyboard focus"),
                    QJsonObject{
                        {QStringLiteral("hint"),
    ...
    m_handler->RegisterMethod(QStringLiteral("cu.key"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
...
void ComputerUseModeApi::registerScrollMethod() {
    m_handler->RegisterMethod(QStringLiteral("cu.scroll"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QWidget* window = getActiveWindow();
