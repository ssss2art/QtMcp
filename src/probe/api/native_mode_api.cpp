...
QWidget* resolveWidgetParam(const QJsonObject& params, const QString& methodName) {
    ...
    if (!widget) {
        ...
        throw JsonRpcException(
            ...
            QStringLiteral("Object is not a widget: %1").arg(objectId),
            QJsonObject{
                {QStringLiteral("objectId"), objectId},
...
void NativeModeApi::registerSystemMethods() {
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.ping"),
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.version"),
        [](const QString& /*params*/) -> QString {
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.modes"),
        [](const QString& /*params*/) -> QString {
            QJsonArray modes;
...
void NativeModeApi::registerObjectMethods() {
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.objects.find"),
        [](const QString& params) -> QString {
            ...
            if (!found) {
                throw JsonRpcException(
                    ...
                    QJsonObject{
                        ...
                        {QStringLiteral("hint"),
                         QStringLiteral("Use qt.objects.tree to see all objects, or qt.objects.findByClass to search by class")}
                    });
            }
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.objects.findByClass"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (className.isEmpty()) {
                throw JsonRpcException(
                    JsonRpcError::kInvalidParams,
                    QStringLiteral("Missing required parameter: className"),
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.objects.info"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.objects.inspect"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.objects.query"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QString className = p[QStringLiteral("className")].toString();
...
void NativeModeApi::registerPropertyMethods() {
    // qt.properties.list
    m_handler->RegisterMethod(QStringLiteral("qt.properties.list"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    // qt.properties.get
    m_handler->RegisterMethod(QStringLiteral("qt.properties.get"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    // qt.properties.set
    m_handler->RegisterMethod(QStringLiteral("qt.properties.set"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (!p.contains(QStringLiteral("value"))) {
                throw JsonRpcException(
                    JsonRpcError::kInvalidParams,
                    QStringLiteral("Missing required parameter: value"),
                    QJsonObject{{QStringLiteral("method"), QStringLiteral("qt.properties.set")}});
            }
...
void NativeModeApi::registerMethodMethods() {
    // qt.methods.list
    m_handler->RegisterMethod(QStringLiteral("qt.methods.list"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    // qt.methods.invoke
    m_handler->RegisterMethod(QStringLiteral("qt.methods.invoke"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (method.isEmpty()) {
                throw JsonRpcException(
                    JsonRpcError::kInvalidParams,
                    QStringLiteral("Missing required parameter: method"),
...
void NativeModeApi::registerSignalMethods() {
    // qt.signals.list
    m_handler->RegisterMethod(QStringLiteral("qt.signals.list"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.signals.subscribe"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QString objectId = p[QStringLiteral("objectId")].toString();
    ...
    m_handler->RegisterMethod(QStringLiteral("qt.signals.unsubscribe"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
...
void NativeModeApi::registerUiMethods() {
    // qt.ui.click
    m_handler->RegisterMethod(QStringLiteral("qt.ui.click"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (button == QStringLiteral("right")) btn = InputSimulator::MouseButton::Right;
            else if (button == QStringLiteral("middle")) btn = InputSimulator::MouseButton::Middle;

            QPoint clickPos;
            if (!pos.isEmpty()) {
                clickPos = QPoint(pos[QStringLiteral("x")].toInt(),
                                  pos[QStringLiteral("y")].toInt());
            }
    ...
    // qt.ui.sendKeys
    m_handler->RegisterMethod(QStringLiteral("qt.ui.sendKeys"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    // qt.ui.screenshot
    m_handler->RegisterMethod(QStringLiteral("qt.ui.screenshot"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            if (fullWindow) {
                ...
                QRect rect(region[QStringLiteral("x")].toInt(),
                           region[QStringLiteral("y")].toInt(),
    ...
    // qt.ui.geometry
    m_handler->RegisterMethod(QStringLiteral("qt.ui.geometry"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    // qt.ui.hitTest
    m_handler->RegisterMethod(QStringLiteral("qt.ui.hitTest"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            ...
            result[QStringLiteral("className")] = obj
...
void NativeModeApi::registerNameMapMethods() {
    // qt.names.register
    m_handler->RegisterMethod(QStringLiteral("qt.names.register"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QString name = p[QStringLiteral("name")].toString();
    ...
    // qt.names.unregister
    m_handler->RegisterMethod(QStringLiteral("qt.names.unregister"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QString name = p[QStringLiteral("name")].toString();
    ...
    // qt.names.load
    m_handler->RegisterMethod(QStringLiteral("qt.names.load"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
    ...
    // qt.names.save
    m_handler->RegisterMethod(QStringLiteral("qt.names.save"),
        [](const QString& params) -> QString {
            auto p = parseParams(params);
            QString filePath = p[QStringLiteral("filePath")].toString();
