...
QJsonObject ResponseEnvelope::wrap(const QJsonValue& result,
                                   const QString& objectId) {
    QJsonObject meta;
    meta[QStringLiteral("timestamp")] =
...
QString ResponseEnvelope::createError(int code, const QString& message,
...
QString ResponseEnvelope::createValidationError(
    const QString& method, const QString& missingParam,
    ...
    return createError(JsonRpcError::kInvalidParams,
                       QStringLiteral("Missing required parameter: %1")
                           .arg(missingParam),
                       data);
}
