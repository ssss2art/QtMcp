...
bool SymbolicNameMap::loadFromFile(const QString& filePath) {
    ...
    if (parseError.error != QJsonParseError::NoError) {
        qWarning() << "SymbolicNameMap: Parse error in" << filePath
    ...
    qDebug() << "SymbolicNameMap: Loaded" << m_nameMap.size()
             << "names from" << filePath;
    return true;
}
...
QJsonArray SymbolicNameMap::validateNames() const {
    ...
    for (auto it = m_nameMap.constBegin(); it != m_nameMap.constEnd(); ++it) {
        ...
        entry[QStringLiteral("valid")] =
            (registry->findById(it.value()) != nullptr);
