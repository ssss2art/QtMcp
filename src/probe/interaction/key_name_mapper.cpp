...
const QHash<QString, Qt::KeyboardModifier>& KeyNameMapper::modifierMap() {
    static const QHash<QString, Qt::KeyboardModifier> map = {
        ...
        {"control_l",   Qt::ControlModifier},
        {"shift",       Qt::ShiftModifier},
        {"shift_l",     Qt::ShiftModifier},
        {"alt",         Qt::AltModifier},
        {"alt_l",       Qt::AltModifier},
        {"super",       Qt::MetaModifier},
        {"super_l",     Qt::MetaModifier},
...
Qt::Key KeyNameMapper::resolve(const QString& name) {
    ...
    if (name.length() == 1) {
        ...
        // Common punctuation
        if (ch == QLatin1Char('-')) return Qt::Key_Minus;
        if (ch == QLatin1Char('=')) return Qt::Key_Equal;
        if (ch == QLatin1Char('[')) return Qt::Key_BracketLeft;
        if (ch == QLatin1Char(']')) return Qt::Key_BracketRight;
        if (ch == QLatin1Char(';')) return Qt::Key_Semicolon;
        if (ch == QLatin1Char('\'')) return Qt::Key_Apostrophe;
        if (ch == QLatin1Char(',')) return Qt::Key_Comma;
        if (ch == QLatin1Char('.')) return Qt::Key_Period;
        if (ch == QLatin1Char('/')) return Qt::Key_Slash;
        if (ch == QLatin1Char('\\')) return Qt::Key_Backslash;
        if (ch == QLatin1Char('`')) return Qt::Key_QuoteLeft;
    }
